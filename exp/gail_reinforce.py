    ...
    def train(self, model: torch.nn.Module, epochs: int, x: torch.tensor, y: torch.tensor, xt: torch.tensor, yt: torch.tensor) -> list:
        ...
        for _ in tqdm(range(epochs), desc="Training"):
            ...
            for batch_idx, (x_batch, y_batch) in enumerate(dl):
                ...
                # if batch_idx == 0:
                #     print("expert judge: ", self.discriminator(x_batch, y_batch[:, 0, [0]]).mean(), "model judge: ", self.discriminator(pi_states, pi_actions).mean())
                #     print("expert reward: ", self.get_reward(x_batch, y_batch[:, 0, [0]]).mean(), "model reward: ", self.get_reward(pi_states, pi_actions).mean())
                ...
                for sim_idx in range(y_batch.shape[1]):
                    ...
                    # if batch_idx == 0 and (sim_idx == 0 or sim_idx == y_batch.shape[1]-1):
                    #     print('sim index:', sim_idx, ', reward:', reward.mean())
